...
public class MultipleTextViewGroup extends RelativeLayout {
	...
	public int getMax_lins() {
		return Max_lins;
	}

	public void setMax_lins(int max_lins) {
		Max_lins = max_lins;
	}

	private int Max_lins =-1;
		...
		if(getMax_lins() != -1) {
			for (int i = 0; i <= getMax_lins()-1; i++) {
				...
				for (int j = 0; j < lineMap.get(i).size(); j++) {
					...
					if (overspread) {
						LayoutParams lp2 = (LayoutParams) tView2
								.getLayoutParams();
